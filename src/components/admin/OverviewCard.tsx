...
const colorMapByClass: Record<string, { iconBg: string; iconText: string; activeRing: string; accentBorder: string }> = {
  ...
  indigo: { iconBg: "bg-orange-50/50", iconText: "text-orange-700", activeRing: "ring-orange-500/10", accentBorder: "border-orange-200" },
  teal: { iconBg: "bg-orange-50/50", iconText: "text-orange-700", activeRing: "ring-orange-500/10", accentBorder: "border-orange-200" },
...
export function OverviewCard({ title, count, icon: Icon, colorClass, className }: OverviewCardProps) {
  ...
  return (
    <div className={cn("bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm transition-all duration-500 group hover:shadow-xl hover:shadow-orange-900/5 hover:border-orange-200 ring-1 ring-slate-100/50", className)}>
      <div className="flex items-center justify-between gap-6 mb-8">
        ...
        <div className="text-4xl font-black text-[#1e4b7a] tracking-tighter transition-all group-hover:scale-110 origin-right duration-500">{count}</div>
      </div>
      <div className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none mt-2 transition-colors group-hover:text-orange-700">{title}</div>
    </div>
  );
}
